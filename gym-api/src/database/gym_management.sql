...
SELECT u.id AS user_id, u.name, u.email, s.instructor_id
FROM users u
JOIN students s ON s.user_id = u.id
WHERE u.name LIKE '%João Pedro%' OR u.email = 'joaopedro@gmail.com';

SELECT id, name, email FROM users WHERE userType = 'instructor' AND (email = 'carlos@gmail.com' OR name LIKE '%Carlos%');

UPDATE students SET instructor_id = <INSTRUTOR_USER_ID> WHERE user_id = <JOAO_USER_ID>;
...
SELECT * FROM user WHERE role = 'instructor' OR role = 'admin';
